...
CONTACTS_CSV = r"C:\Users\srija\OneDrive\Desktop\meetingagent\contacts.csv"

# Vexa API configuration
VEXA_API_KEY = os.getenv("VEXA_API_KEY", "ntIzjlr9jkBGEeTDJz1aDVQBdiIeRHx5GaUwKfhg")
...
def create_bot():
    ...
    # Extract meeting link from the request
    ...
    # Google Meet URLs are typically in format: https://meet.google.com/abc-defg-hij
    meeting_id = None
    
    # Use regex to extract the meeting ID from the URL
    import re
    
    # Pattern to match Google Meet URLs
    meet_pattern = r'https://meet\.google\.com/([a-zA-Z0-9\-]+)'
    match = re.search(meet_pattern, meeting_link)
    
    if match:
        meeting_id = match.group(1)
    else:
        # If regex doesn't match, try to extract the last part of the URL
        try:
            meeting_id = meeting_link.split('/')[-1]
            # Validate that it looks like a Google Meet ID (contains hyphens)
            if '-' not in meeting_id:
                raise ValueError("Invalid meeting ID format")
        except:
            return jsonify({"error": "Invalid Google Meet URL format. Please provide a valid Google Meet link."}), 400
    
    print(f"Extracted meeting ID: {meeting_id} from URL: {meeting_link}")
    
    url = f"{VEXA_BASE_URL}/bots"
    headers = {
    ...
    payload = {
        ...
        "native_meeting_id": meeting_id,  # Use the extracted meeting ID
        "meeting_url": meeting_link,      # Also send the full URL
        "language": "en",
        "bot_name": "Yobot"
    }

    print(f"Sending payload to Vexa API: {json.dumps(payload, indent=2)}")
    ...
    try:
        ...
        # Check if the bot was created successfully
        if response.status_code == 200 or response.status_code == 201:
            # Store the meeting ID for later use in fetching transcript
            session['current_meeting_id'] = meeting_id
            print(f"Bot created successfully for meeting ID: {meeting_id}")
...
def fetch_transcript():
    # Get meeting ID from session (stored when bot was created) or from query parameter
    meeting_id = session.get('current_meeting_id') or request.args.get('meeting_id')
    
    if not meeting_id:
        return jsonify({"error": "No meeting ID found. Please create a bot first."}), 400
    
    print(f"Fetching transcript for meeting ID: {meeting_id}")
    ...
    try:
        ...
        return jsonify({
            ...
            "meeting_id": meeting_id  # Include meeting ID in response
        }), 200
    except ValueError:
        print("Non-JSON response:", response.text)
        return jsonify({"error": "Invalid JSON response", "text": response.text}), 500
...
def stop_bot():
    ...
    print(f"Stopping bot for meeting ID: {meeting_id}")
    
    url = f"{VEXA_BASE_URL}/bots/google_meet/{meeting_id}"
    headers = {
        "X-API-Key": VEXA_API_KEY
    }
    ...
    if response.status_code == 200:
        # Clear the meeting ID from session when bot is stopped
...
if __name__ == '__main__':
    ...
    if not os.getenv("VEXA_API_KEY"):
        print("Warning: VEXA_API_KEY not found in environment variables. Vexa integration will be disabled.")
